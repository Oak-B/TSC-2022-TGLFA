...
class Data:
    ...
    def construct_high_order_graph(self, Graph):
        Graph_dense = Graph.to_dense()
        i_matrix = torch.eye(self.max_user_id + self.max_item_id)
        print(i_matrix.shape, Graph_dense.shape)
        graph_new = Graph_dense
        graphs = [Graph]
        for layer in range(self.n_layers - 1):
            graph_new = torch.mm(graph_new, Graph_dense)
            graphs.append(graph_new)

        high_order_graph = i_matrix
        for i in range(self.n_layers):
            high_order_graph += graphs[i]

        high_order_graph = high_order_graph / (self.n_layers + 1)
        high_order_graph = high_order_graph.to(device)

        idx = torch.nonzero(high_order_graph).T
        data = high_order_graph[idx[0], idx[1]]
        print(idx[0][0], idx[1][0], data[0])
        print(len(data))

        return high_order_graph
    ...
    def build_graph(self):
        ...
        self.high_order_graph = self.construct_high_order_graph(self.Graph)
