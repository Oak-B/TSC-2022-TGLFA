...
from TGLFA.model.TGLFA import TGLFA
from TGLFA.model.TGLFA import eval
from TGLFA.model.TGLFA import MSEloss
from torch.autograd import Variable
from TGLFA.utils.LoadData import Data
from TGLFA.utils.smo import *
from TGLFA.model.MSE import MSE
from TGLFA.utils import globalData
import os
import warnings
...
def main(LR, L2, batch_size, size, mom, drop_prob, name, update_manner, optimizer_type, val_or_test, layer_num, idx, p, n_layers):
    ...
    TGLFA_model = TGLFA(layer_sizes=[data.max_user_id] + size,
                                        drop_prob=drop_prob, lr = LR, num_items=data.max_item_id, graph = data.high_order_graph,
                                        num_users = data.max_user_id, p = p, n_layers=n_layers)

    TGLFA_model = TGLFA_model.cuda()

    if optimizer_type == "SGD":
        optimizer = optim.SGD(TGLFA_model.parameters(), lr=LR, weight_decay=L2)
        if update_manner == "normal":
            save_dir = "result/SGD/" + name + "/"
        else:
            save_dir = "result/NSGD/" + name + "/"
    elif optimizer_type == "Adam":
        optimizer = optim.Adam(TGLFA_model.parameters(), lr=LR, weight_decay=L2)
        if update_manner == "normal":
    ...
    for i in range(num_epochs):
        ...
        # for name, param in TGLFA_model.named_parameters():
        #     print(name)
        ...
        for mb in data.train_iterate_one_epoch():
            ...
            optimizer.zero_grad()
            outputs = TGLFA_model.forward(Ap, Aj, Ax, drop_flag, isTrain)
            #outputs = nn.ReLU()(outputs)
            loss = MSE.apply(outputs, Ax)

            loss.backward()
            optimizer.step()
            # TGLFA_model.embedding_item.weight = TGLFA_model.embedding_item_cache.weight
        ...
        for mb_train, mb_test in data.test_iterate_one_epoch():
            ...
            outputs = TGLFA_model(Ap, Aj, Ax, drop_flag, isTrain)
